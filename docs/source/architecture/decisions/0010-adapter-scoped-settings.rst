...
:doc:`0009-layered-configuration`; the rest of ADR 0009 remains in effect,
subject to what ADR 0011 supersedes there.
...
the call site. The ``configure()`` form shown in decision 1 is superseded with
decision 8. Decisions 2, 3, 4, 6 and 7 remain in effect: the tiers,
source-major precedence, package-wide environment variables, the host-scoped
key, and the adapter names.
...
1. **The configuration file gains one table per adapter**, beside the existing
   ...
   .. note::
      ...
      The file table remains in effect; the ``configure()`` form above is
      superseded by :doc:`0011-configuration-profiles` along with decision 8.
      One block still configures several adapters at once, now as
      ``configure(NgwmnConfiguration(concurrency=4),
      WqpConfiguration(retries=2))``.

2. **The top-level tier remains.** An adapter table *overrides* it per key; it
...
4. **Adapter-scoped settings get no environment variables.** Every entry in
   ...
   namespace nobody could remember.
...
5. **Each adapter's schema is a** ``TypedDict``. Its ``__annotations__`` *are*
   ...
   *Superseded by* :doc:`0011-configuration-profiles`. The schema is now a
   ...
   schema" reason -- what changed is where it is defined. A ``TypedDict`` had
   to be declared centrally to annotate a central keyword, which put a Water
...
8. **Each adapter is a named, typed parameter on** ``configure()``, annotated
   ...
   *Superseded by* :doc:`0011-configuration-profiles`. ``configure()`` takes
   ...
   class rather than by a keyword. The type checking remains -- a setting an
   ...
   the keyword that does not exist. What the change achieves is that
   ``configure()`` no longer enumerates the adapters at all: that enumeration
   was the roster this ADR left listed in four places.

Consequences
------------

- **A caller can throttle one adapter without throttling the rest** --
...
- **The schema stops being a separate mechanism.** Choosing ``TypedDict`` over
  ...
  validator and ``configuration`` adds no runtime classes. *Superseded with
  decision 5*: the classes exist, and are defined with their adapters rather
  than in the leaf.
...
- **A configuration object is still refused, but on narrower grounds than ADR
  ...
  *Withdrawn by* :doc:`0011-configuration-profiles`, which took the remaining
  ...
  open, and a dataclass turned out to provide what a mapping could not: an
  instance records which adapter it targets, so the caller stops naming one
...
- **``stall_timeout`` is added to the chain.** ``API_USGS_STALL_TIMEOUT`` was
  read directly from ``os.environ``, so it could not be set by a block or the
  file and never appeared in ``show_configuration()`` -- a gap in ADR 0009's
  own claim that every setting resolves through one chain. It is package-wide by
...
- The ``wateruse`` module is renamed ``nwdc`` in a separate change; the service
