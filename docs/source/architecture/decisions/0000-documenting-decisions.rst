...
way, when a name cannot express it -- an ordering that matters, an upstream
quirk, an early return that looks removable. One or two lines, adjacent to the
code they explain. A comment longer than that is describing something wider
than the lines beneath it, and belongs in one of the venues below.
...
- **Write from the reader's position.** A citation replaces an argument only if
  the sentence left behind is understandable on its own. Prose that assumes the
  reader has the cited record already open, or relies on a term the glossary
  does not define, has moved the cost of the duplication rather than removed it.
- **An accepted ADR is not edited to reverse its meaning.** A later decision
  ...
  ``Notes``, and its ``Status`` says the record was amended, so a reader sees
...
this record had put in the right venue and that only the author could follow:
