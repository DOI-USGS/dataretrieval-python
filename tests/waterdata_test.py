...
def test_multi_value_chunked_emits_3d_cartesian_product():
    """Three chunkable dims, each forced to split → exhaustive cartesian
    product across all three. Verifies the planner's halving loop handles
    N>2 dims uniformly and the wrapper's ``itertools.product`` enumerates
    ...
    assert len(sites_seen) > 1, "sites dim was not split"
    assert len(pcodes_seen) > 1, "pcodes dim was not split"
    assert len(stats_seen) > 1, "stats dim was not split"

    # Cartesian shape: # sub-calls == product of unique chunks across dims
    expected = len(sites_seen) * len(pcodes_seen) * len(stats_seen)
    assert len(calls) == expected, (
        f"expected {expected} cartesian-product sub-calls, got {len(calls)}"
    )
...
def test_plan_chunks_rejects_non_positive_max_chunks():
    """``max_chunks < 1`` is meaningless and would silently bypass the
    cap on the no-chunking-needed path (initial plan total = 1 and the
    in-loop check only runs after a split). Reject early."""
    args = {"monitoring_location_id": ["A", "B", "C", "D"]}
    with pytest.raises(ValueError, match="max_chunks must be >= 1"):
        _plan_chunks(args, _fake_build, url_limit=1000, max_chunks=0)
    with pytest.raises(ValueError, match="max_chunks must be >= 1"):
        _plan_chunks(args, _fake_build, url_limit=1000, max_chunks=-5)
