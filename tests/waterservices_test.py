...
def test_get_info(requests_mock):
    ...
    mock_request(requests_mock, request_url, response_file_path)
    df, md = get_record(sites=["01491000", "01645000"], parameterCd="00618", start='2020-02-14', end='2020-02-15', service='site')
    assert type(df) is DataFrame
    assert df.size == 24
    assert md.url == request_url
...
def test_get_qwdata(requests_mock):
    ...
    request_url = 'https://nwis.waterdata.usgs.gov/nwis/qwdata?agency_cd=USGS&format={}' \
                  ...
                  '&qw_sample_wide=separated_wide'.format(format, site)
...
def test_get_pmcodes(requests_mock):
    ...
    request_url = 'https://nwis.waterdata.usgs.gov/nwis/pmcodes/pmcodes?radio_pm_search=pm_search' \
                  ...
                  '&show=parameter_group_nm&show=casrn&show=srsname&show=parameter_units&format={}'.format(format)
