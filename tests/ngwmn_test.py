...
def test_get_ngwmn_sites():
    df, md = get_ngwmn_sites(state_name="Wisconsin", limit=10)
...
def test_get_ngwmn_sites_skip_geometry():
    df, _ = get_ngwmn_sites(monitoring_location_id=_SITE, skip_geometry=True)
    assert isinstance(df, DataFrame)
    assert "geometry" not in df.columns


def test_get_ngwmn_water_level():
    df, md = get_ngwmn_water_level(monitoring_location_id=_SITE)
    assert isinstance(df, DataFrame)
    assert len(df) > 0
...
def test_get_ngwmn_water_level_datetime_subsets():
    full, _ = get_ngwmn_water_level(monitoring_location_id=_SITE)
    windowed, _ = get_ngwmn_water_level(
...
def test_get_ngwmn_providers():
    df, md = get_ngwmn_providers(state="WI")
    assert isinstance(df, DataFrame)
    assert len(df) > 0
...
def test_get_ngwmn_lithology():
    df, _ = get_ngwmn_lithology(monitoring_location_id=_LITH_SITE)
    assert isinstance(df, DataFrame)
    assert len(df) > 0
    assert (df["monitoring_location_id"] == _LITH_SITE).all()


def test_get_ngwmn_well_construction():
    df, _ = get_ngwmn_well_construction(monitoring_location_id=_SITE)
    assert isinstance(df, DataFrame)
    assert len(df) > 0
    assert (df["monitoring_location_id"] == _SITE).all()


def test_ngwmn_multi_site_chunks_and_unions():
    ...
    one, _ = get_ngwmn_water_level(monitoring_location_id=_SITE)
    many, _ = get_ngwmn_water_level(
