...
It aliases the public names; it is not a second name for the module object. This is a
...
``wateruse.get_wateruse`` leaves ``nwdc``'s global unchanged (and so has no
effect on anything ``nwdc`` does internally), and ``wateruse._WATERUSE_HOST``
does not exist. Code that monkeypatches, or that uses a private name, must
...
rather than this module, so ``import dataretrieval`` emits no warning. The warning
is emitted only for code that names ``wateruse`` itself.
"""
...
#: dated-removal convention :mod:`dataretrieval.nwis` uses.
