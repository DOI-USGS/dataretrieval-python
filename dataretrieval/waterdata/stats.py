...
# ``_handle_nesting`` builds GeoDataFrames when geopandas is present, so
# this module needs its own bound ``gpd`` name. Import it under the same guard
# the engine uses; when geopandas is absent ``gpd`` is left unbound
# (``GEOPANDAS`` is ``False``, so the stats path never touches it). Tests patch
# ``stats.gpd`` with ``create=True`` to cover both environments.
try:
